...
export default function ContextMenu(): JSX.Element | null {
  ...
  // --- List Actions ---
  const handleListRenameSubmit = async (): Promise<void> => {
    const { id, name } = data
    const trimmed = renameValue.trim()
    if (trimmed && trimmed !== name) {
      const updated = await window.api.updateList(id, { name: trimmed })
      useListStore.getState().setLists(lists.map(l => l.id === id ? updated : l))
    }
    hideContextMenu()
  }

  const handleListDeleteConfirmed = async (): Promise<void> => {
    const { id } = data
    ...
    <>
            ...
            {renaming ? (
              <div className="px-3 py-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') handleListRenameSubmit()
                    if (e.key === 'Escape') { setRenaming(false); setRenameValue('') }
                  }}
                  className="w-full bg-surface text-text-primary text-sm rounded-input px-2 py-1 outline-none border border-accent"
                />
                <div className="flex gap-1 mt-2">
                  <button
                    onClick={handleListRenameSubmit}
                    className="flex-1 py-1 text-[11px] bg-accent text-white rounded-input hover:opacity-90 transition-opacity"
                  >
                    {t('update_pin').split(' ')[0]}
                  </button>
                  <button
                    onClick={() => { setRenaming(false); setRenameValue('') }}
                    className="flex-1 py-1 text-[11px] text-text-secondary hover:text-text-primary transition-colors"
                  >
                    {t('cancel')}
                  </button>
                </div>
              ...
              <button
                onClick={() => { setRenaming(true); setRenameValue(data.name) }}
      ...
      {confirmDeleteList && (
        <ConfirmModal
          ...
          onConfirm={handleListDeleteConfirmed}
